...
# Executa as tabelas SQL da aplicação (para Unity Comprovantes se for instalação nova)
if [ "$SYSTEM_CHOICE" -eq 11 ] && [ $IS_UPDATE -eq 0 ] && [ -f "$APP_DIR/schema.sql" ] && [ ! -z "$DB_PASSWORD" ]; then
...
EOL
    ...
    # Lógica Genérica (ITL, Rastreae, Opa, Unity, iWedding, StreamControl, Unity DVR, Unity Tax, VooSimples, Unity Comprovantes)
    cd $APP_DIR
